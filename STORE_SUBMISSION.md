...
The browser extension packages do not include an AI model, API key, or OAuth token. AI reply and draft tools use the Xtension Connector installed separately by the user on Windows. The no-admin connector can use the official Codex App Server with the user's ChatGPT sign-in, the user's existing Claude Code subscription session, or a configured Ollama server; it bundles no model. Voice dictation was removed in 0.6.7: the packages contain no microphone or audio-capture code.
...
```text
JoDevelop does not receive or store website content, AI requests, or AI responses. PDF export processes visible X/Twitter content locally in the browser. AI features are off by default and start only after the user turns them on in Options. When enabled, selected text and useful visible context go through the local open-source connector only to the engine selected for that attempt: OpenAI Codex, Claude Code, or a configured localhost/private-network Ollama server. Contextual suggestions may use the same context after the user deliberately focuses an empty supported reply field. No provider API key or OAuth token is stored by the extension, and Xtension never submits a reply automatically.
```
...
- Verify that the Connector detects Codex/ChatGPT, Claude Code, and the configured Ollama model independently.
