...
$inputExe = Join-Path $distDir "bridge-input\XtensionInput.exe"
...
foreach ($file in @($bridgeExe, $hostExe, $inputExe, $installerProject)) {
...
foreach ($file in @($bridgeExe, $hostExe, $inputExe)) {
...
Copy-Item -LiteralPath $inputExe -Destination (Join-Path $payloadDir "XtensionInput.exe") -Force
